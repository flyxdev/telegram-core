...
EXT_POCKETCORE_CHUNKUTILS_VERSION="0.1.0"
...
echo "[PocketCore] PHP compiler for Linux, MacOS and Android"
DIR="$(pwd)"
...
COMPILE_POCKETCORE_CHUNKUTILS="no"
COMPILE_GD="no"
...
while getopts "::t:j:srdlxff:ugnva:" OPTION; do
	...
	case $OPTION in
		...
		f)
			...
			;;
		u)
			echo "[opt] Will compile with PocketCore-ChunkUtils C extension for Anvil"
			COMPILE_POCKETCORE_CHUNKUTILS="yes"
			;;
		g)
...
if [ "$IS_CROSSCOMPILE" == "yes" ]; then
	...
	#TODO: add aarch64 platforms (ios, android, rpi)
		...
		if [ `getconf LONG_BIT` == "64" ]; then
			...
			echo "[ERROR] PocketCore is no longer supported on 32-bit systems"
			exit 1
		fi
	fi
fi
...
if [ "$COMPILE_POCKETCORE_CHUNKUTILS" == "yes" ]; then
	get_github_extension "pocketcore-chunkutils" "$EXT_POCKETCORE_CHUNKUTILS_VERSION" "dktapps" "PocketCore-C-ChunkUtils"
	HAS_POCKETCORE_CHUNKUTILS=--enable-pocketcore-chunkutils
else
	HAS_POCKETCORE_CHUNKUTILS=""
fi
...
date >> "$DIR/install.log" 2>&1
echo "[PocketCore] You should start the server now using \"./start.sh\"."
echo "[PocketCore] If it doesn't work, please send the \"install.log\" file to the Bug Tracker."
